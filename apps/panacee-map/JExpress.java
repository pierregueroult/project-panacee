...
public final class JExpress {
   ...
   * Lancer depuis apps/panacee-map/ : java JExpress.java
   * Puis ouvrir                     : http://localhost:8080/apps/panacee-map/
   */
  static void main() {
    var app = express();
    var projectRoot = Path.of("../..").toAbsolutePath().normalize();
    app.use(staticFiles(projectRoot));
    app.listen(8080);

    System.out.println("Panacée map → http://localhost:8080/apps/panacee-map/");
  }
}