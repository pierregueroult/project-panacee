...
void export_fitness_csv(const Fitness *fitness, const char *path)
{
    ...
    fprintf(f, "%d,%d,%d,%.2f,%d,%.2f,%.2f,%d,%.2f\n", fitness->hospital_count, fitness->uhc_count,
            fitness->distant_resident_count, fitness->distant_resident_percent, fitness->distant_town_count,
            fitness->distant_town_percent, fitness->fitness_score, fitness->fitness_count, fitness->fitness_average);
    fclose(f);
...
void export_result_csv(const Individual *result, const Town *towns, const int *insee_to_idx, const char *path)
{
    ...
    for (i = 0; i < result->size; i++)
    {
        ...
        if (j >= 0)
        {
            fprintf(f, "%d,\"%s\",%s,\"%s\",%d,%d,%d\n", towns[j].insee, towns[j].name, towns[j].department_code,
                    towns[j].department_name, towns[j].inhabitants_count,
                    towns[j].inhabitants_count > THRESHOLD_UHC ? 1 : 0, result->hospitals[i].beds_count);
        }
    }
    fclose(f);
                             ...
                             int **coverage, const int *coverage_size, const char *path)
{
    ...
    for (i = 0; i < town_count; i++)
    {
        ...
        int assigned_insee = hidx >= 0 ? result->hospitals[hidx].insee : -1;
        fprintf(f, "%d,\"%s\",%s,\"%s\",%d,%d\n", towns[i].insee, towns[i].name, towns[i].department_code,
                towns[i].department_name, towns[i].inhabitants_count, assigned_insee);
    }
    fclose(f);
